...
-(NSData*) recv: (int) length {
    ...
    if(bytesRead == -1) {
        @throw [[SteamCondenserException alloc] initWithMessage:[NSString stringWithFormat:@"Could not read from socket: %s", strerror(errno)]];
    }
...
-(bool) select:(int) timeout {
    ...
    if(selectResult == -1) {
        @throw [[SteamCondenserException alloc] initWithMessage:[NSString stringWithFormat:@"select() failed: %@", strerror(errno)]];
    }
...
-(int) send:(NSData*) data {
    ...
    if(sendResult == -1) {
        @throw [[SteamCondenserException alloc] initWithMessage:[NSString stringWithFormat:@"Send failed: %s", strerror(errno)]];
    }
